...
1. **Exposes a GitHub repo as MCP tools.** Stdio transport, JSON-RPC framing, installs into any MCP-compatible client.
   ...
   - **LLM tools (opt-in via `LLM_API_KEY`):** `summarize_issue`, `triage_pr`. Vendor-agnostic — set `LLM_PROVIDER=groq` (default) and supply a key. Both tools fetch the relevant GitHub artefact, send it to the configured provider in a delimited prompt, and return a structured JSON response (summary, suggested labels, risk areas, next steps).
   - **Composite tool (requires both `WRITES_ENABLED=true` and `LLM_API_KEY`):** `propose_fix`. Reads an issue + comments, optionally fetches caller-supplied `relevantPaths`, prompts the LLM for a complete patch (full file contents), then runs `create_branch` → `commit_files` → `open_pr` (draft, with `Closes #N` in the body). End-to-end "issue → draft PR" handoff. The PR is always opened as a draft; a human reviews and merges.
