...
function mapHttpStatus(status: number, message: string): AppError {
  switch (status) {
    ...
    default:
      return AppError.githubApi(status, message || 'openai embeddings error', ENDPOINT);
  }
}
...
function toEmbedResponse(
  ...
  if (data === undefined || data.length === 0) {
    return err(AppError.internal('openai embeddings response missing data[]'));
  }
  const ordered = [...data].sort((a, b) => (a.index ?? 0) - (b.index ?? 0));
  const embeddings = ordered.map((entry) => entry.embedding ?? []);
  if (embeddings.some((vec) => vec.length === 0)) {
    return err(AppError.internal('openai embeddings response contained empty vector'));
  }
...
export function createOpenAiEmbeddingProvider(deps: OpenAiEmbeddingDeps): EmbeddingProvider {
  ...
  return {
    ...
    async embed(req: EmbedRequest): Promise<Result<EmbedResponse, AppError>> {
      ...
      const fetched = await tryCatch(
        ...
        (cause) =>
          AppError.internal(
            `openai embeddings request failed: ${(cause as Error)?.message ?? 'unknown'}`,
            cause,
          ),
      );
      ...
      const parsed = await tryCatch(
        () => response.json() as Promise<OpenAiEmbeddingResponse>,
        (cause) => AppError.internal('openai embeddings response not json', cause),
      );
